...
class EnvironmentMesh:
    ...
    def _split_loc(self, point):
        ...
        min_dcx = self.config["region"]["cell_width"] / (
            2 ** (self.config["splitting"]["split_depth"])
        )
        min_dcy = self.config["region"]["cell_height"] / (
            2 ** (self.config["splitting"]["split_depth"])
        )
    ...
    def tie_northern_cellbox_ng(self, north_ext_cellboxes, north_int_cellboxes):
        ...
        for cellbox_l in north_ext_cellboxes:
            ...
            for cellbox_s in north_int_cellboxes:
                ...
                neighbour_case = self.neighbour_graph.get_neighbour_case_bounds(
    ...
    def get_cellboxes_within_bounds(self, bounds):
        ...
        for cellbox in self.agg_cellboxes:
            ...
            if (
                cb_bounds.get_long_min() >= bounds.get_long_min()
                and cb_bounds.get_long_max() <= bounds.get_long_max()
                and cb_bounds.get_lat_min() >= bounds.get_lat_min()
                and cb_bounds.get_lat_max() <= bounds.get_lat_max()
            ):
    ...
    def get_cellboxes_north_of_bounds(self, bounds):
        ...
        for cellbox in self.agg_cellboxes:
            if (
                cellbox.get_bounds().get_lat_min() == bounds.get_lat_max()
                and cellbox.get_bounds().get_long_max() >= bounds.get_long_min()
                and cellbox.get_bounds().get_long_min() <= bounds.get_long_max()
            ):
    ...
    def get_cellboxes_south_of_bounds(self, bounds):
        ...
        for cellbox in self.agg_cellboxes:
            if (
                cellbox.get_bounds().get_lat_max() == bounds.get_lat_min()
                and cellbox.get_bounds().get_long_max() >= bounds.get_long_min()
                and cellbox.get_bounds().get_long_min() <= bounds.get_long_max()
            ):
    ...
    def get_cellboxes_east_of_bounds(self, bounds):
        ...
        for cellbox in self.agg_cellboxes:
            if (
                ...
                and cellbox.get_bounds().get_lat_min() >= bounds.get_lat_min()
                and cellbox.get_bounds().get_lat_max() <= bounds.get_lat_max()
            ):
    ...
    def get_cellboxes_west_of_bounds(self, bounds):
        ...
        for cellbox in self.agg_cellboxes:
            if (
                cellbox.get_bounds().get_long_max() == bounds.get_long_min()
                and cellbox.get_bounds().get_lat_min() >= bounds.get_lat_min()
                and cellbox.get_bounds().get_lat_max() <= bounds.get_lat_max()
            ):
    ...
    def to_tif(self, params_file, path):
        ...
        extent = [
            self.bounds.get_long_min(),
            self.bounds.get_lat_min(),
            self.bounds.get_long_max(),
            self.bounds.get_lat_max(),
        ]
