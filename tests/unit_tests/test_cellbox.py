...
def min_cellbox(standard_test_bounds):
    ...
    het_splitting_conds = {"threshold": 0.5, "upper_bound": 1, "lower_bound": 0}
    return create_cellbox(
        standard_test_bounds, splitting_conds=[het_splitting_conds], min_dp=99999999
    )
...
def test_split():
    parent_cellbox = create_cellbox(Boundary([-10, 10], [-10, 10]), id=0, parent=None)
...
def test_create_splitted_cell_boxes():
    parent_cellbox = create_cellbox(Boundary([-10, 10], [-10, 10]), id=1, parent=None)
