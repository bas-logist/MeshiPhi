...
class TestEnvMesh(unittest.TestCase):
    ...
    def test_load_from_json(self):
        self.assertEqual(
            self.loaded_env_mesh.bounds.get_bounds(), self.env_mesh.bounds.get_bounds()
        )

        self.assertEqual(
            len(self.loaded_env_mesh.agg_cellboxes), len(self.env_mesh.agg_cellboxes)
        )
        self.assertEqual(
    ...
    def test_to_tif(self):
        ...
        # exporting to tif. If GDAL is not installed, we skip this test.
            ...
            self.skipTest(
                "GDAL not available - install with: conda install -c conda-forge gdal"
            )
