...
@pytest.mark.parametrize("boundary_fixture,poly_string", [
    ("arbitrary_boundary", "POLYGON ((30 10, 30 20, 40 20, 40 10, 30 10))"),
    ("meridian_boundary", "POLYGON ((-10 -50, -10 -40, 10 -40, 10 -50, -10 -50))"),
    ("antimeridian_boundary", "MULTIPOLYGON (((170 -50, 170 -40, 180 -40, 180 -50, 170 -50)), ((-180 -50, -180 -40, -170 -40, -170 -50, -180 -50)))"),
...
@pytest.mark.parametrize("date_string,day_offset", [
    ('TODAY', 0),
    ('TODAY - 5', -5),
    ('TODAY + 5', 5)
])
...
@pytest.mark.parametrize("lat_range,long_range,time_range,description", [
...
@pytest.mark.parametrize("boundary_fixture,expected_bounds", [
    ("arbitrary_boundary", [[30.0, 10.0], [30.0, 20.0], [40.0, 20.0], [40.0, 10.0], [30.0, 10.0]]),
    ("meridian_boundary", [[-10.0, -50.0], [-10.0, -40.0], [10.0, -40.0], [10.0, -50.0], [-10.0, -50.0]]),
    ("antimeridian_boundary", [[170.0, -50.0], [170.0, -40.0], [-170.0, -40.0], [-170.0, -50.0], [170.0, -50.0]]),
    ("equatorial_boundary", [[30.0, -10.0], [30.0, 10.0], [40.0, 10.0], [40.0, -10.0], [30.0, -10.0]]),
])
...
@pytest.mark.parametrize("boundary_fixture,expected_cx", [
...
@pytest.mark.parametrize("boundary_fixture,expected_cy", [
...
@pytest.mark.parametrize("boundary_fixture,expected_height", [
...
@pytest.mark.parametrize("boundary_fixture,expected_width", [
    ("arbitrary_boundary", 10),
...
@pytest.mark.parametrize("boundary_fixture,expected_dcx", [
...
@pytest.mark.parametrize("boundary_fixture,expected_dcy", [
    ("arbitrary_boundary", 5),
    ("meridian_boundary", 5),
...
@pytest.mark.parametrize("boundary_fixture,expected_min", [
    ("arbitrary_boundary", 10),
...
@pytest.mark.parametrize("boundary_fixture,expected_max", [
...
@pytest.mark.parametrize("boundary_fixture,expected_size", [
...
@pytest.mark.parametrize("boundary_fixture,poly_wkt", [
    ("arbitrary_boundary", "POLYGON ((30 10, 30 20, 40 20, 40 10, 30 10))"),
    ("meridian_boundary", "POLYGON ((-10 -50, -10 -40, 10 -40, 10 -50, -10 -50))"),
    ("antimeridian_boundary", "MULTIPOLYGON (((170 -50, 170 -40, 180 -40, 180 -50, 170 -50)), ((-180 -50, -180 -40, -170 -40, -170 -50, -180 -50)))"),
    ("equatorial_boundary", "POLYGON ((30 -10, 30 10, 40 10, 40 -10, 30 -10))"),
...
@pytest.mark.parametrize("boundary_fixture,expected_poly_string", [
    ("arbitrary_boundary", "POLYGON ((30 10, 30 20, 40 20, 40 10, 30 10))"),
    ("meridian_boundary", "POLYGON ((-10 -50, -10 -40, 10 -40, 10 -50, -10 -50))"),
    ("antimeridian_boundary", "MULTIPOLYGON (((170 -50, 170 -40, 180 -40, 180 -50, 170 -50)), ((-180 -50, -180 -40, -170 -40, -170 -50, -180 -50)))"),
    ("equatorial_boundary", "POLYGON ((30 -10, 30 10, 40 10, 40 -10, 30 -10))"),
